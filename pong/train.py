...
from env import PongEnv, ReplayBuffer
...
def train(
    ...
    if render_every_n > 0:
        env_render = PongEnv(render=True)
    else:
        env_render = None
    ...
    for eps in range(1, num_episodes + 1):
        ...
        for step in range(max_steps_per_episode):
            ...
            if env_render is not None and eps % render_every_n == 0:
                env_render.ball_x = env.ball_x
                env_render.ball_y = env.ball_y
                env_render.ball_vx = env.ball_vx
                env_render.ball_vy = env.ball_vy
                env_render.agent_y = env.agent_y
                env_render.bot_y = env.bot_y
                env_render.render()
        ...
        if eps % 10 == 0:
            ...
            print(f"Episode {eps:4d} | TotalSteps {total_steps:7d} | Epsilon {epsilon:.3f} | AvgReward(last100) {avg_reward:.3f}")
